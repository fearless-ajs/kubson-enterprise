...
class UserDatabaseObject
{
    ...
    public static function count_all()
    {
      ...
      //function that counts records in a table
    public static function count_men_by_cat($cat)
    {
        global $database;
        $sql = "SELECT COUNT(*) FROM " . static::$table_name . " WHERE category = '{$cat}'";
        $result_set = $database->query($sql);
        $row = $database->fetch_array($result_set);
        return array_shift($row);
    }
    //function that counts records in a table
    public static function count_by_name_and_cat($name, $cat)
    {
        global $database;
        $sql = "SELECT COUNT(*) FROM " . static::$table_name . " WHERE Match(name) Against('{$name}') AND category = '{$cat}'";
        $result_set = $database->query($sql);
        $row = $database->fetch_array($result_set);
        return array_shift($row);
    }
    ...
    public function update_by_Patient_ID($ID = "")
    {
        ...
        //the update password function
    public function update_cart_quantity($value, $productID)
    {
        global $database;
        //Don't forget your SQL syntax and good habits
        // - UPDATE table SET key='value', key='value' WHERE condition
